...
class MonitoredAccount(db.Model):
    ...
    notification_preferences = relationship(
        "NotificationPreference", back_populates="account", cascade="all, delete-orphan"
    )
...
class NotificationPreference(db.Model):
    ...
    account = relationship(
        "MonitoredAccount", back_populates="notification_preferences"
    )
    ...
    try:
        ...
        if notification_preferences:
            for pref_type, enabled in notification_preferences.items():
                pref = NotificationPreference(type=pref_type, enabled=enabled)
...
def update_notification_preferences(handle, preferences):
    ...
    try:
        ...
        logger.info(
            f"Current preferences for {handle}: {[{'type': p.type, 'enabled': p.enabled} for p in account.notification_preferences]}"
        )
        ...
        for pref in account.notification_preferences:
            if pref.type in preferences:
                ...
                logger.info(
                    f"Updating {pref.type} preference for {handle}: {old_value} -> {pref.enabled}"
                )
                updated = True
                db.session.add(pref)
        ...
        for pref_type, enabled in preferences.items():
            if pref_type not in existing_types:
                logger.info(
                    f"Adding new {pref_type} preference for {handle}: {enabled}"
                )
                new_pref = NotificationPreference(
                    account=account, type=pref_type, enabled=enabled
                )
        ...
        logger.info(
            f"Updated preferences for {handle}: {[{'type': p.type, 'enabled': p.enabled} for p in account.notification_preferences]}"
        )
...
def remove_monitored_account(identifier, by_did=False):
    ...
    try:
        ...
        if not account:
            return {
                "error": f"Account not found with {'DID' if by_did else 'handle'}: {identifier}"
            }
...
def mark_post_notified(account_did: str, post_id: str) -> bool:
    ...
    try:
        ...
        existing = NotifiedPost.query.filter_by(
            account_did=account_did, post_id=post_id
        ).first()
        ...
        notification = NotifiedPost(account_did=account_did, post_id=post_id)
