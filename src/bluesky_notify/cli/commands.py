...
from bluesky_notify.core.database import (add_monitored_account, db,
...
@click.option(
    "--desktop/--no-desktop", default=True, help="Enable/disable desktop notifications"
)
@click.option(
    "--email/--no-email", default=False, help="Enable/disable email notifications"
)
    ...
    with app.app_context():
        ...
        try:
            ...
            if "error" in result:
                ...
                console.print(
                    f"[green]Successfully added {account_info['display_name'] or handle} to monitored accounts[/green]"
                )
...
def list():
    ...
    with app.app_context():
        ...
        for account in accounts:
            status = (
                "[green]Active[/green]" if account.is_active else "[red]Inactive[/red]"
            )
            prefs = {p.type: p.enabled for p in account.notification_preferences}
            console.print(
                f"{account.handle} ({account.display_name or 'No display name'}) - {status}"
            )
            console.print(
                f"  Notifications: Desktop: {prefs.get('desktop', False)}, Email: {prefs.get('email', False)}"
            )
...
def toggle(handle):
    ...
    with app.app_context():
        if toggle_account_status(handle):
            console.print(
                f"[green]Successfully toggled monitoring status for {handle}[/green]"
            )
        else:
...
def update(handle, desktop, email):
    ...
    with app.app_context():
        if update_notification_preferences(handle, desktop, email):
            console.print(
                f"[green]Successfully updated preferences for {handle}[/green]"
            )
        else:
...
def remove(handle):
    ...
    with app.app_context():
        if remove_monitored_account(handle):
            console.print(
                f"[green]Successfully removed {handle} from monitored accounts[/green]"
            )
        else:
